...
angular.module('APIServiceApp').directive('iziAccount', function () {
    return {
        restrict: "EA",
        replace: true,
        transclude: true,
        ...
        controller: ['$scope', '$rootScope', '$element', '$attrs', '$http', '$window', '$timeout', '$log', '$mdDialog', '$mdToast', '$animate', '$firebaseObject', '$firebaseArray', 'APIService', function ($scope, $rootScope, $element, $attrs, $http, $window, $timeout, $log, $mdDialog, $mdToast, $animate, $firebaseObject, $firebaseArray, APIService) {
            ...
            function onDeviceReady() {
                if (window.device) {
                    ...
                    if (!$scope.clientUrl) APIService.set.clientUrl('http://ffpizza.izipass.pro');else APIService.set.clientUrl($scope.clientUrl);
                        ...
                        confTable.$loaded().then(function (data) {
                            ...
                            if (result[0]) {
                                ...
                                $scope.data.$loaded().then(function (data) {
                                    ...
                                    angular.element(document).find('head').append("<style type='text/css'>" + buildStyleFromData($scope.customization) + angular.element('#izi-style').html().replace(/#123456/g, $scope.customization.styling.mainColor).replace(/#654321/g, $scope.customization.styling.secondaryColor).replace(/#321654/g, $scope.customization.styling.bgColor ? $scope.customization.styling.bgColor : 'transparent') + "</style>");
            ...
            function buildStyleFromData(data) {
                ...
                if (data.styling.bgColor) {
                    ...
                    if (data.styling.bgColor !== "transparent" && data.styling.bgColor !== "#ffffff" && data.styling.bgColor !== "#fff") {
                        fidItemStyle = ".izi-account .fid-item-title," + ".izi-account .fid-item-title + div b," + ".izi-account .fid-item-title + input { color: " + blackOrWhite(data.styling.bgColor) + " !important; }" + ".izi-account .card h4 small, .izi-account .card.error h4 { color: " + data.styling.mainColor + " !important; }" + ".izi-account .alert p, .izi-account .barcode-container small, .izi-account .card h4 { color: " + blackOrWhite(blackOrWhite(data.styling.bgColor)) + " !important;  font-family: " + secondaryFontName + ", Helvetica, Arial, sans-serif !important; }";
                    }
                }

                return "@import url(" + data.styling.mainFont + ");" + "@import url(" + data.styling.secondaryFont + ");" + ".izi-account h1, .izi-account h2, .izi-account h3:not(.fid-item-title) { color: " + data.styling.mainColor + " !important; font-family:" + mainFontName + ", Helvetica, Arial, sans-serif !important; }" + ".izi-account h4, .izi-account h5, .izi-account p, .izi-account a, .izi-account small, .izi-account p, .izi-account label { color: " + data.styling.secondaryColor + " !important; font-family: " + secondaryFontName + ", Helvetica, Arial, sans-serif !important; }" + ".izi-account .fid-item-title { font-family:" + mainFontName + ", Helvetica, Arial, sans-serif !important; }" + ".izi-account input { color: " + data.styling.secondaryColor + " !important; font-family: " + secondaryFontName + ", Helvetica, Arial, sans-serif !important; }" + ".izi-account .fid-item-title + div b, .izi-account .fid-item-title + input { font-family:" + secondaryFontName + ", Helvetica, Arial, sans-serif !important; }" + ".izi-account a, .izi-account a:hover { color: " + data.styling.mainColor + " !important; } " + ".izi-account button { color: " + blackOrWhite(data.styling.bgColor) + " !important;  font-family: " + secondaryFontName + ", Helvetica, Arial, sans-serif !important; }" + fidItemStyle;
            }
            ...
            $scope.toast = function (message) {
                $mdToast.show($mdToast.simple().content(message).position($scope.getToastPosition()).hideDelay(1500));
            };
            ...
            $scope.login = function (barcode) {
                ...
                if (barcode) bc = barcode;else if ($scope.form.barcode) bc = $scope.form.barcode;
            ...
            $scope.submitRegister = function () {
                ...
                APIService.actions.register(obj).then(function () {
                    ...
                    if (error.status === 500) $window.alert('Cette carte est déjà enregistrée !');else $window.alert('Une erreur ' + error.status + ' est survenue !');
                });
            };
